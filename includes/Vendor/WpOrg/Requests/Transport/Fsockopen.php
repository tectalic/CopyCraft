 ...
 */
namespace OM4\CopyCraft\Vendor\WpOrg\Requests\Transport;
...
use OM4\CopyCraft\Vendor\WpOrg\Requests\Ssl;
use OM4\CopyCraft\Vendor\WpOrg\Requests\Transport;
use OM4\CopyCraft\Vendor\WpOrg\Requests\Utility\CaseInsensitiveDictionary;
...
final class Fsockopen implements Transport
{
    ...
    public function request($url, $headers = [], $data = [], $options = [])
    {
        ...
        if (isset($url_parts['scheme']) && \strtolower($url_parts['scheme']) === 'https') {
            ...
            $context_options = ['verify_peer' => \true, 'capture_peer_cert' => \true];
            $verifyname = \true;
            // SNI, if enabled (OpenSSL >=0.9.8j)
            // phpcs:ignore PHPCompatibility.Constants.NewConstants.openssl_tlsext_server_nameFound
            if (\defined('OPENSSL_TLSEXT_SERVER_NAME') && \OPENSSL_TLSEXT_SERVER_NAME) {
                $context_options['SNI_enabled'] = \true;
    ...
    private static function format_get($url_parts, $data)
    {
        if (!empty($data)) {
            ...
            $url_parts['query'] = \trim($url_parts['query'], '&');
        }
