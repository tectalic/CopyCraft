...
namespace OM4\CopyCraft\Vendor\Tectalic\OpenAi\Models;

use JsonSerializable;
use OM4\CopyCraft\Vendor\Spatie\DataTransferObject\DataTransferObjectError;
use OM4\CopyCraft\Vendor\Spatie\DataTransferObject\FlexibleDataTransferObject;
use stdClass;
    ...
    public function __construct(array $parameters = [])
    {
        foreach (static::DEPRECATED as $originalName) {
            if (isset($parameters[$originalName])) {
                \trigger_error("Property `{$originalName}` is deprecated", \E_USER_DEPRECATED);
            }
        }
        ...
        foreach ($validators as $field => $validator) {
            ...
            if (!isset($parameters[$field]) && !\in_array($field, static::REQUIRED, \true)) {
                unset($this->{$field});
                continue;
            }
            if (!isset($parameters[$field]) && !$validator->isNullable) {
            ...
            if (!$validator->isValidType($value)) {
                throw DataTransferObjectError::invalidType(static::class, $field, $validator->allowedTypes, $value);
            }
            $this->{$field} = $value;
