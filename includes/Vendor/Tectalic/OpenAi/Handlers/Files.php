...
namespace OM4\CopyCraft\Vendor\Tectalic\OpenAi\Handlers;
...
use OM4\CopyCraft\Vendor\Tectalic\OpenAi\Models\Files\CreateRequest;
use OM4\CopyCraft\Vendor\Tectalic\OpenAi\Models\Files\CreateResponse;
use OM4\CopyCraft\Vendor\Tectalic\OpenAi\Models\Files\DeleteResponse;
use OM4\CopyCraft\Vendor\Tectalic\OpenAi\Models\Files\ListResponse;
use OM4\CopyCraft\Vendor\Tectalic\OpenAi\Models\Files\RetrieveResponse;
use Throwable;
final class Files
{
    /** @var Client */
    ...
    {
        $url = '/files';
        $this->setRequest($this->client->get($url, null, []));
    ...
    {
        $url = '/files';
        $this->setRequest($this->client->post($url, \is_array($body) ? new CreateRequest($body) : $body, ['Content-Type' => 'multipart/form-data']));
    ...
    {
        $url = \sprintf('/files/%s', $fileId);
        $this->setRequest($this->client->delete($url, null, []));
    ...
    private function parseResponse(ResponseInterface $response) : array
    {
        ...
        $data = (array) \json_decode($body, \true);
        if (\json_last_error()) {
            throw new ClientException('Failed to parse JSON response body: ' . \json_last_error_msg());
        }
        return $data;
    }
    /**
    ...
    public function toModel() : object
    {
        if ($this->getResponse()->getStatusCode() < 200 || $this->getResponse()->getStatusCode() >= 300) {
            throw new ClientException(\sprintf('Unsuccessful response. HTTP status code: %s (%s).', $this->getResponse()->getStatusCode(), $this->getResponse()->getReasonPhrase()));
        }
        $class = $this->modelType;
        try {
            return new $class($this->parseResponse($this->getResponse()));
        } catch (Throwable $e) {
            throw new ClientException('Response body parse failed. See previous exception for details.', 0, $e);
        }
    }
    /**
