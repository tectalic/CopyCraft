...
namespace OM4\CopyCraft\Vendor\Http\Message\MultipartStream;
...
use OM4\CopyCraft\Vendor\Http\Discovery\StreamFactoryDiscovery;
use OM4\CopyCraft\Vendor\Http\Message\StreamFactory as HttplugStreamFactory;
...
class MultipartStreamBuilder
{
    ...
    public function __construct($streamFactory = null)
    {
        ...
        if (null !== $streamFactory) {
            throw new \LogicException(\sprintf('First arguemnt to the constructor of "%s" must be of type "%s", "%s" or null. Got %s', __CLASS__, StreamFactoryInterface::class, HttplugStreamFactory::class, \is_object($streamFactory) ? \get_class($streamFactory) : \gettype($streamFactory)));
        }
