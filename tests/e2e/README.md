...
Automated end-to-end tests for CopyCraft.

Based on https://github.com/woocommerce/woocommerce-e2e-boilerplate/.
...
We recommend using Docker for running tests locally in order for the test environment to match the setup on GitHub Actions (where Docker is also used for running tests). [An official WordPress Docker image](https://github.com/docker-library/docs/blob/master/wordpress/README.md) is used to build the site.
...
Note that it can take 30-60 seconds for the provisioning script (wp-content/plugins/copycraft/tests/e2e/docker/initialize.sh) to fully run. That script installs WooCommerce and configures it. Until complete the test site will simply be a standard WordPress installation with no WooCommerce or CopyCraft.

Use the following Admin user details to login:
...
Or the following customer user details to login as a WooCommerce customer:
```
Username: customer
PW: password
```
...
```bash
npm run test:e2e .tests/e2e/specs/merchant.test.js
```
...
```
TRAVIS_MARIADB_VERSION=10.5.3 PHP_VERSION=7.4.5 WP_VERSION=5.4.1 npm run docker:up
```
